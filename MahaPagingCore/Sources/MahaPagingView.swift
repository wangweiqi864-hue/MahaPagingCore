...
open class MahaPagingView: UIView {
    ...
    private weak var delegate: MahaPagingViewDelegate?
    private var tableHeaderContainerView: UIView!
    private let cellIdentifier = "cell"
    private let listContainerType: MahaPagingListContainerType
    private var listCache = [String:MahaPagingViewListViewDelegate]()
    ...
    open func reloadData() {         currentList = nil
    ...
    //仅用于处理设置了pinSectionHeaderVerticalOffset，又添加了MJRefresh的下拉刷新。这种情况会导致MahaPagingView和MJRefresh来回设置contentInset值。针对这种及其特殊的情况，就内部特殊处理了。通过下面的判断条件，来判定当前是否处于下拉刷新中。请勿让pinSectionHeaderVerticalOffset和下拉刷新设置的contentInset.top值相同。
    //具体原因参考：https://github.com/pujiaxin33/MahaPagingView/issues/203
